...
contextBridge.exposeInMainWorld('xeroApi', {
    // App initialization
    onAppInit:      (cb: (data: unknown) => void) => ipcRenderer.on('xero-app-init', (_e, data) => cb(data)),
    onSetView:      (cb: (view: string, args: unknown[]) => void) => ipcRenderer.on('update-main-window-view', (_e, view, ...args) => cb(view, args)),
    onNavData:      (cb: (data: unknown) => void) => ipcRenderer.on('send-nav-data', (_e, data) => cb(data)),
    onStatus:       (cb: (status: unknown) => void) => ipcRenderer.on('send-app-status', (_e, status) => cb(status)),
    onInfoData:     (cb: (data: unknown) => void) => ipcRenderer.on('send-info-data', (_e, data) => cb(data)),
    onPromptString: (cb: (req: unknown) => void) => ipcRenderer.on('prompt-string-request', (_e, req) => cb(req)),
    onStatusOverlay:(cb: (visible: boolean, text: string) => void) => ipcRenderer.on('set-status-overlay', (_e, visible, text) => cb(visible, text)),
    ...
    promptStringResponse: (id: string, value: string | undefined) => ipcRenderer.invoke('prompt-string-response', id, value),
